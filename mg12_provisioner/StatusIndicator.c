...
void led_off_timer_on_timeout(sl_sleeptimer_timer_handle_t *handle,
                              void *data) {
  (void)handle;
  (void)data;
...
void status_indicator_on_provisioning() {
  ...
  sl_sleeptimer_start_periodic_timer_ms(
      &led_timer, 500, led_timer_on_timeout, NULL, 0,
...
void status_indicator_on_success() {
  ...
  sl_sleeptimer_start_timer_ms(
      &led_timer, 3000, led_timer_on_timeout, NULL, 0,
      SL_SLEEPTIMER_NO_HIGH_PRECISION_HF_CLOCKS_REQUIRED_FLAG);
}
...
void status_indicator_on_failed() {
  ...
  sl_simple_led_turn_off(sl_led_led0.context);
  sl_sleeptimer_start_periodic_timer_ms(
      &led_timer, 100, led_timer_on_timeout, NULL, 0,
      SL_SLEEPTIMER_NO_HIGH_PRECISION_HF_CLOCKS_REQUIRED_FLAG);
  sl_sleeptimer_start_timer_ms(
      &led_off_timer, 3000, led_off_timer_on_timeout, NULL, 0,
      SL_SLEEPTIMER_NO_HIGH_PRECISION_HF_CLOCKS_REQUIRED_FLAG);
}
